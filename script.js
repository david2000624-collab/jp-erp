...
const translations = {
  zh: {
    ...
    addOrder: "新增訂單", addProduct: "新增商品", addPurchase: "新增進貨", addPackage: "新增包裹", addShipment: "新增出貨", addCustomer: "新增客戶", addPayment: "新增付款", addShippingAdvance: "新增運費代墊費用", addSplitPayment: "新增拆款", addSettlementPayment: "新增結算付款",
  ...
  ja: {
    ...
    addOrder: "注文追加", addProduct: "商品追加", addPurchase: "仕入追加", addPackage: "荷物追加", addShipment: "出荷追加", addCustomer: "顧客追加", addPayment: "支払い追加", addShippingAdvance: "送料立替費用を追加", addSplitPayment: "分配支払い追加", addSettlementPayment: "精算支払い追加",
...
const config = {
  ...
  settlementPayments: { prefix: "T", formId: "settlementPaymentForm", money: ["amount"] }
};
...
function normalize(data) {
  ...
  merged.splitPayments = (merged.splitPayments || []).map((payment) => {
    ...
    return { ...payment, payer, receiver, note: payment.note || "", amount: Number(payment.amount || 0), createdAt: payment.createdAt || new Date().toISOString() };
  });
...
function settlementSummary() {
  ...
  const remainingProfitBalance = profitBalance - paidProfitBalance;
  const finalBalance = remainingAdvanceBalance + remainingProfitBalance;
  return {
    advances,
    splits,
    advanceBalance,
...
function exportExcel() {
  try {
    ...
    const tables = [
      ...
      excelTable(text("splitPayments"), ["ID", text("paidBy"), text("receivedBy"), text("splitAmount"), "TWD", text("paymentNote")], data.splitPayments.map((payment) => [payment.id, payment.payer, payment.receiver, excelMoney(payment.amount), excelMoney(jpyToTwd(payment.amount)), payment.note])),
...
function renderProfit() {
  ...
  document.querySelector("#splitPaymentList").innerHTML = state.data.splitPayments.map((payment) => `<article class="item-card"><div class="item-top"><strong>${payment.payer} ${text("payTo")} ${payment.receiver}</strong><span class="pill blue">${money(payment.amount)}</span></div><span class="meta">${shortDate(payment.createdAt)}${payment.note ? ` / ${payment.note}` : ""}</span>${actions("splitPayments", payment.id)}</article>`).join("") || emptyList();
...
function formValues(form, collection) {
  ...
  if ((collection === "splitPayments" || collection === "settlementPayments") && values.payer === values.receiver) {
...
document.querySelector("#settlementPaymentForm").addEventListener("submit", (event) => upsertFromForm(event, "settlementPayments"));
